...
public class BancoContabilidade {
    ...
    public void BancoDebito(String CodFato, int Valor, String Detalhes, String Data) {
        try {
            Conexao.AbrirConexao();
            String SQL = String.format("INSERT INTO BANCO(COD_FATO, CREDITO, DEBITO, DETALHES, DATA) VALUES('%s', '%d', '%d', '%s', '%s')", CodFato, 0, Valor, Detalhes, Data);
            int ValorCaixa = Conexao.getConexao().createStatement().executeUpdate(SQL);
            System.out.println("Boa cadastrou no BD, debito");
        } catch (SQLException e) {
            throw new RuntimeException(e);
